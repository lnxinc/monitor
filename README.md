...
### Reliability Tuning
Customize heartbeat behaviour and alert throttling via the following environment variables:

- `HEARTBEAT_MAX_ATTEMPTS` (default `3`): how many times the collector retries transient failures.
- `HEARTBEAT_INITIAL_BACKOFF_MS` (default `200`): initial retry delay in milliseconds before exponential backoff.
- `HEARTBEAT_BACKOFF_MULTIPLIER` (default `2.0`): multiplier applied to each subsequent retry delay.
- `ALERT_DEDUPE_TTL_SECONDS` (default `300`): window in seconds for deduplicating identical incident notifications.
- `ALERT_FLOOD_LIMIT` (default `10`): maximum messages a channel can receive within the flood window before suppression.
- `ALERT_FLOOD_WINDOW_SECONDS` (default `60`): flood-control window size in seconds.
