...
class UnifiSiteManager
{
    ...
    public function syncIntoDatabase(): array
    {
        ...
        foreach ($sites as $site) {
            ...
            $device = Device::where('external_source', 'unifi_site_manager')
                ->where('external_id', $site['external_id'])
                ->first();

            if (! $device) {
                $device = Device::where('organization_id', $org->id)
            ...
            $attributes = [
                ...
                'status' => $site['is_online'] ? DeviceStatus::Online : DeviceStatus::Offline,
            ...
            if (! $site['is_online'] || $site['has_internet_issue']) {
                $openExists = Incident::where('device_id', $device->id)
                    ->where('status', 'open')
                    ->exists();
                if (! $openExists) {
    ...
    private function normalize(array $raw): array
    {
        ...
        if (! filter_var($ip, FILTER_VALIDATE_IP)) {
            ...
            if (is_array($wans)) {
                foreach ($wans as $wan) {
                    $cand = $wan['ipv4'] ?? null;
                    if (is_string($cand) && filter_var($cand, FILTER_VALIDATE_IP)) {
                        $ip = $cand;
                        break;
                    }
                }
        ...
        if (! filter_var($ip, FILTER_VALIDATE_IP)) {
            ...
            if (is_array($ipAddrs)) {
                foreach ($ipAddrs as $cand) {
                    if (is_string($cand) && filter_var($cand, FILTER_VALIDATE_IP) && strpos($cand, ':') === false) {
                        $ip = $cand;
                        break;
                    }
                }
