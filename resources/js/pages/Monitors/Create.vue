...
watch(
    ...
    (value) => {
        ...
        if (value === 'sip') {
            form.config = {
                ...form.config,
                timeout: form.config.timeout ?? 10,
                port: form.config.port ?? 5060,
                phone_number: form.config.phone_number ?? '',
            };
        }
        if (value === 'webhook') {
            form.config = {};
                    ...
                    <form @submit.prevent="form.post('/monitors')" class="space-y-6">
                        ...
                        <div v-if="form.type === 'sip'" class="space-y-4">
                            <div class="space-y-2">
                                ...
                                <input
                                    id="sip-phone"
                                    v-model="form.config.phone_number"
                                    required
                                    type="text"
                                    placeholder="+1234567890 or 1000"
                                    class="flex h-10 w-full rounded-md border border-input bg-background px-3 text-sm"
                                />
                                <div v-if="form.errors['config.phone_number']" class="text-sm text-destructive">
                                    {{ form.errors['config.phone_number'] }}
                                </div>
                            </div>
                            <div class="grid gap-4 md:grid-cols-2">
                                ...
                                <div class="space-y-2">
                                    ...
                                    <input
                                        ...
                                        class="flex h-10 w-full rounded-md border border-input bg-background px-3 text-sm"
                                    />
                                    <div v-if="form.errors['config.timeout']" class="text-sm text-destructive">
                                        {{ form.errors['config.timeout'] }}
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div v-if="form.type === 'webhook'" class="rounded-md border border-dashed p-4 text-sm text-muted-foreground">
                            The webhook token will be generated after saving. Use this monitor when an external service pushes health events to the
                            platform.
                        </div>
