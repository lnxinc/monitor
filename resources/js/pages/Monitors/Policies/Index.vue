...
const destroyPolicy = (id: number) => {
    if (confirm('Delete this policy?')) router.delete(`/policies/${id}`);
};
</script>
        ...
        <div class="flex h-full flex-1 flex-col gap-6 p-6">
            <div class="flex items-center justify-between">
                ...
                <Link href="/policies/create"
                    ><Button><Plus class="mr-2 h-4 w-4" /> Add Policy</Button></Link
                >
            </div>

            <div v-if="props.policies.length === 0" class="py-12 text-center">
            ...
            <div v-else class="space-y-3">
                <Card v-for="p in props.policies" :key="p.id">
                    <CardHeader class="flex flex-row items-center justify-between gap-3">
                        <div>
                            ...
                            <div class="text-xs text-muted-foreground">
                                Threshold: {{ p.down_threshold }} • {{ p.notify_on_recovery ? 'Notify on recovery' : 'No recovery notification' }}
                            </div>
                    ...
                    <CardContent>
                        <div class="flex items-center justify-between">
                            <Link :href="`/policies/${p.id}/edit`"
                                ><Button variant="outline" size="sm"><Edit class="h-4 w-4" /></Button
                            ></Link>
                            <Button variant="outline" size="sm" class="text-destructive" @click="destroyPolicy(p.id)"
