...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Edit', href: `/policies/${props.policy.id}/edit` },
];

const form = useForm({
    monitor_id: String(props.policy.monitor_id),
    down_threshold: props.policy.down_threshold,
    notify_after_seconds: props.policy.notify_after_seconds || 0,
    repeat_interval_minutes: props.policy.repeat_interval_minutes || '',
    notify_on_recovery: props.policy.notify_on_recovery,
    enabled: props.policy.enabled,
    channel_ids: props.policy.channels.map((c) => c.id),
});
