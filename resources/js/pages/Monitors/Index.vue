        ...
        <div class="flex h-full flex-1 flex-col gap-6 p-6">
            <div class="flex items-center justify-between">
                ...
                <Link href="/monitors/create"
                    ><Button><Plus class="mr-2 h-4 w-4" />Add Monitor</Button></Link
                >
            </div>

            <div v-if="props.monitors.length === 0" class="py-12 text-center">
                    ...
                    <CardHeader class="flex flex-row items-center justify-between">
                        ...
                        <span
                            :class="[
                                ...
                                m.status === 'up' ? 'text-green-600' : m.status === 'down' ? 'text-red-600' : 'text-gray-500',
                            ]"
                            >{{ m.status }}</span
                        >
                    </CardHeader>
                    <CardContent class="space-y-3">
                        <div class="truncate text-sm text-muted-foreground">{{ m.display_target ?? m.url }}</div>
                        ...
                        <div class="text-xs text-muted-foreground">
                            Last check: {{ m.last_checked_at ? new Date(m.last_checked_at).toLocaleString() : '—' }}
                        </div>
                        <div class="flex items-center justify-between">
                            <div class="text-xs text-muted-foreground">
                                {{ m.last_status_code ?? '—' }} • {{ m.last_response_time_ms ? m.last_response_time_ms + 'ms' : '—' }}
                            </div>
                            <div class="flex gap-2">
                                <Link :href="`/monitors/${m.id}`"><Button variant="outline" size="sm">View</Button></Link>
                                <Link :href="`/monitors/${m.id}/edit`"
                                    ><Button variant="outline" size="sm"><SquarePen class="h-4 w-4" /></Button
                                ></Link>
                                <Button v-if="m.type !== 'webhook'" variant="outline" size="sm" @click="runNow(m)" title="Run check"
                                    ><RefreshCw class="h-4 w-4"
                                /></Button>
                                <Button variant="outline" size="sm" class="text-destructive" @click="destroyMonitor(m.id)" title="Delete"
