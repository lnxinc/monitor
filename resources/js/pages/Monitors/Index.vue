...
interface Props {
  monitors: Array<{ id:number; name:string; url:string; frequency_minutes:number; status:'up'|'down'|'unknown'; last_checked_at:string|null; last_status_code:number|null; last_response_time_ms:number|null }>;
}
...
const runNow = (id: number) => {
  router.post(`/monitors/${id}/run`, {}, { preserveScroll: true, onSuccess: () => router.reload({ only: ['monitors'] }) });
};
    ...
    <div class="flex h-full flex-1 flex-col gap-6 p-6">
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-muted-foreground">HTTP/SSL checks for external resources</p>
        </div>
      ...
      <div v-else class="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        <Card v-for="m in props.monitors" :key="m.id">
          ...
          <CardContent class="space-y-3">
            <div class="text-sm text-muted-foreground truncate">{{ m.url }}</div>
            <div class="text-xs text-muted-foreground">Every {{ m.frequency_minutes }} min</div>
            ...
            <div class="flex items-center justify-between">
              ...
              <div class="flex gap-2">
                ...
                <Button variant="outline" size="sm" @click="runNow(m.id)" title="Run check"><RefreshCw class="h-4 w-4" /></Button>
