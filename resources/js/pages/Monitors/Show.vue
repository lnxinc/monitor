    ...
    <div class="flex h-full flex-1 flex-col gap-6 p-6">
      <div class="flex items-center justify-between">
        <div>
          <div class="flex items-center gap-2">
            ...
            <Badge variant="outline" class="uppercase text-xs">{{ typeLabel }}</Badge>
          </div>
          <p class="text-muted-foreground">{{ displayTarget }}</p>
          <p v-if="isWebhook" class="text-xs text-muted-foreground mt-1">Webhook token: {{ props.monitor.webhook_token ?? 'generated after save' }}</p>
        </div>
        <div class="flex items-center gap-2">
      ...
      <Card>
        ...
        <CardContent>
          ...
          <div v-if="props.monitor.consecutive_failures" class="text-xs text-muted-foreground mt-2">Consecutive failures: {{ props.monitor.consecutive_failures }}</div>
        </CardContent>
      </Card>
      ...
      <Card v-if="props.monitor.ssl_expires_at">
        ...
        <CardContent class="grid gap-4 md:grid-cols-3">
          <div>
            ...
            <div class="text-sm font-medium">{{ props.monitor.ssl_valid_from ? new Date(props.monitor.ssl_valid_from).toLocaleString() : '—' }}</div>
          </div>
          <div>
            <div class="text-sm text-muted-foreground">Expires</div>
            <div class="text-sm font-medium">{{ props.monitor.ssl_expires_at ? new Date(props.monitor.ssl_expires_at).toLocaleString() : '—' }}</div>
          </div>
          <div>
        ...
        <CardContent>
          <div class="space-y-2 max-h-[400px] overflow-y-auto pr-2">
            <div v-for="c in props.monitor.checks" :key="c.id" class="flex flex-col gap-2 border-b pb-3">
              ...
              <pre v-if="c.meta" class="text-[11px] font-mono bg-muted/40 rounded p-2 whitespace-pre-wrap break-all">{{ formatJson(c.meta) }}</pre>
              <pre v-if="c.payload" class="text-[11px] font-mono bg-muted/20 rounded p-2 whitespace-pre-wrap break-all">{{ formatJson(c.payload) }}</pre>
            </div>
          </div>
        </CardContent>
      </Card>
