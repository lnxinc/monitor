...
interface Props { organizations: Opt[]; devices: Opt[]; monitors: Opt[]; maintenance: Window }
const props = defineProps<Props>();

const breadcrumbs: BreadcrumbItem[] = [ { title:'Dashboard', href:'/dashboard' }, { title:'Maintenance', href:'/maintenance' }, { title:'Edit', href:`/maintenance/${props.maintenance.id}/edit` } ];

const form = useForm({ scope: props.maintenance.scope, ref_id: String(props.maintenance.ref_id || ''), start_at: props.maintenance.start_at.slice(0,16), end_at: props.maintenance.end_at.slice(0,16), reason: props.maintenance.reason || '' });
        ...
        <CardContent class="space-y-4">
          ...
          <div class="space-y-2" v-if="form.scope !== 'global'">
            ...
            <select v-model="form.ref_id" class="flex h-10 w-full rounded-md border px-3 py-2 text-sm">
              ...
              <option v-for="o in (form.scope==='organization'?props.organizations:form.scope==='device'?props.devices:props.monitors)" :key="o.id" :value="o.id">{{ o.name }}</option>
            </select>
          </div>
