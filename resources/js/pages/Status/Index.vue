...
<template>
    ...
    <div class="min-h-screen bg-background text-foreground">
        <div class="mx-auto max-w-5xl p-6">
            <div class="mb-6">
                <h1 class="text-3xl font-bold">LNX Monitor Service Status</h1>
                <p class="text-muted-foreground">Uptime over the last 24 hours</p>
            </div>

            <div class="space-y-3">
                <div v-for="m in props.monitors" :key="m.id" class="flex items-center justify-between rounded-lg border p-4">
                    <div class="min-w-0">
                        <div class="flex items-center gap-3">
                            <div
                                :class="[
                                    'h-3 w-3 rounded-full',
                                    m.status === 'up' ? 'bg-green-500' : m.status === 'down' ? 'bg-red-500' : 'bg-gray-400',
                                ]"
                            ></div>
                            <div class="truncate font-medium">{{ m.name }}</div>
                        </div>
                        <div class="truncate text-xs text-muted-foreground">{{ m.url }}</div>
                    </div>
                    <div class="flex items-center gap-6">
                        ...
                        <div class="text-right">
                            ...
                            <div class="text-sm text-muted-foreground">{{ formatRelativeTime(m.last_checked_at) }}</div>
                        </div>
                        <div class="text-right">
