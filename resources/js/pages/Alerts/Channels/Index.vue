...
interface Channel { id: number; name: string; type: 'email'|'slack_webhook'; is_active: boolean; organization?: { id: number; name: string } | null }
interface Props { channels: Channel[] }
const props = defineProps<Props>();

const breadcrumbs: BreadcrumbItem[] = [ { title:'Dashboard', href:'/dashboard' }, { title:'Alert Channels', href:'/channels' } ];
const destroyChannel = (id: number) => { if (confirm('Delete this channel?')) router.delete(`/channels/${id}`) };
  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="flex h-full flex-1 flex-col gap-6 p-6">
      <div class="flex items-center justify-between">
        ...
        <Link href="/channels/create"><Button><Plus class="h-4 w-4 mr-2" /> Add Channel</Button></Link>
      </div>

      <div v-if="props.channels.length === 0" class="text-center py-12">
      ...
      <div v-else class="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        <Card v-for="c in props.channels" :key="c.id">
          ...
          <CardContent>
            <div class="flex items-center justify-between">
              <Link :href="`/channels/${c.id}/edit`"><Button variant="outline" size="sm"><Edit class="h-4 w-4" /></Button></Link>
              <Button variant="outline" size="sm" class="text-destructive" @click="destroyChannel(c.id)"><Trash2 class="h-4 w-4" /></Button>
            </div>
          </CardContent>
        </Card>
      </div>
