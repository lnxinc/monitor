...
interface Channel { id: number; name: string; type: 'email'|'slack_webhook'; is_active: boolean; organization_id?: number | null; config?: any }
interface Props { organizations: Org[]; channel: Channel }
const props = defineProps<Props>();

const breadcrumbs: BreadcrumbItem[] = [ { title:'Dashboard', href:'/dashboard' }, { title:'Alert Channels', href:'/channels' }, { title:'Edit', href:`/channels/${props.channel.id}/edit` } ];

const form = useForm({ organization_id: String(props.channel.organization_id || ''), name: props.channel.name, type: props.channel.type, config_email: props.channel.config?.email || '', config_webhook_url: props.channel.config?.webhook_url || '', is_active: props.channel.is_active });
const submit = () => {
